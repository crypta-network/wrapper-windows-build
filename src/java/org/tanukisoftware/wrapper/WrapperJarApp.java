    ...
    implements WrapperListener, Runnable
{
    ...
    static
    {
        ...
        if ( "true".equals( System.getProperty( "wrapper.use_sun_encoding" ) ) ) {
            ...
            if ( ( sunStdoutEncoding != null ) && !sunStdoutEncoding.equals( System.getProperty( "file.encoding" ) ) ) {
                ...
                try
                {
                    c_outInfo = new WrapperPrintStream( System.out, false, sunStdoutEncoding, "WrapperJarApp: " );
                    c_outError = new WrapperPrintStream( System.out, false, sunStdoutEncoding, "WrapperJarApp Error: " );
                    c_outDebug = new WrapperPrintStream( System.out, false, sunStdoutEncoding, "WrapperJarApp Debug: " );
                    streamsSet = true;
                }
        ...
        if ( !streamsSet )
        {
            c_outInfo = new WrapperPrintStream( System.out, "WrapperJarApp: " );
            c_outError = new WrapperPrintStream( System.out, "WrapperJarApp Error: " );
            c_outDebug = new WrapperPrintStream( System.out, "WrapperJarApp Debug: " );
        }
    }
                ...
                if ( !m_initFailed )
                {
                    ...
                    if ( !m_initFailed )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                c_outDebug.println( "Jar Main-Class: " + mainClassName );
                            }
                            ...
                            if ( ( classPath != null ) && ( !classPath.equals( "" ) ) )
                            {
                                if ( WrapperManager.isDebugEnabled() )
                                {
                                    c_outDebug.println( WrapperManager.getRes().getString("Jar Classpath: {0}", classPath ) );
                                }
                                ...
                                if ( !m_initFailed )
                                {
                                    if ( WrapperManager.isDebugEnabled() )
                                    {
                                        c_outDebug.println( WrapperManager.getRes().getString("    Classpath[0]=") + classURLs[0] );
                                    }
                                    ...
                                    for ( int i = 1; st.hasMoreTokens(); i++ )
                                    {
                                        ...
                                        if ( !m_initFailed )
                                        {
                                            if ( WrapperManager.isDebugEnabled() )
                                            {
                                                c_outDebug.println( WrapperManager.getRes().getString("    Classpath[{0}]=", new Integer( i ) ) + classURLs[i] );
                                            }
                                        }
                                    }
                                }
                            ...
                            else
                            {
                                if ( WrapperManager.isDebugEnabled() )
                                {
                                    c_outDebug.println( WrapperManager.getRes().getString( "Jar Classpath: Not specified." ) );
                                }
    ...
    public void run()
    {
        ...
        try
        {
            if ( WrapperManager.isDebugEnabled() )
            {
                c_outDebug.println( WrapperManager.getRes().getString("invoking main method" ) );
            }
            ...
            if ( WrapperManager.isDebugEnabled() )
            {
                c_outDebug.println(  WrapperManager.getRes().getString("main method completed" ) );
            }
        ...
        c_outInfo.println();
        c_outError.println( WrapperManager.getRes().getString( "Encountered an error running main:" ) );
    ...
    public Integer start( String[] args )
    {
        ...
        synchronized(this)
        {
            // Override the appArgs, they are usually the same as what was set in the constructor.
            m_appArgs = args;
    ...
    public int stop( int exitCode )
    {
        if ( WrapperManager.isDebugEnabled() )
        {
            c_outDebug.println( "stop(" + exitCode + ")" );
        }
     ...
     *-------------------------------------------------------------*/
    /**
     * Returns the application arguments meant for the main method of the Main application class.
     *
    ...
    public static void main( String args[] )
    {
        ...
        // Create the WrapperJarApp instance which parses parses the full
        //  list of arguments into a jar file and arguments for its main class.
        WrapperJarApp wrapperJarApp = new WrapperJarApp( args );
        
        // Start the application.  If the JVM was launched from the native
        //  Wrapper then the application will wait for the native Wrapper to
        //  call the application's start method.  Otherwise the start method
        //  will be called immediately.
        WrapperManager.start( wrapperJarApp, wrapperJarApp.getAppArgs() );
        
        // This thread returns and ends, the WrapperManager will start the application after the Wrapper has
