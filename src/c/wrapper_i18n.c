...
/**
 * This function is not defined on some Unix platforms, so lets define it.
 */
int _tcsnicmp(const wchar_t *string1, const wchar_t *string2, size_t count) {
    wint_t c1, c2;
    ...
    do {
        c1 = (wint_t)towlower(*string1++);
        c2 = (wint_t)towlower(*string2++);
        if ((c1 == 0) || (c1 != c2)) {
            return c1 - c2;
        }
    } while (--count > 0);
...
/**
 * Wrapper function for usleep() allowing to suspends thread execution for more than 1 second.
 *  According to the doc, usec must be smaller than 1000000 on some systems (this is the case on z/OS).
 *  This function splits the suspension time into intervals no longer than USLEEP_MAX and repeat as
 *  many times as needed.
 *
 * @param usec suspension time in microsoconds
 *
 * @return 0 on success, -1 on error.
 */
int wrapperUsleep(unsigned long usec) {
    unsigned long chunk;
    
    while(usec > 0) {
        chunk = usec > USLEEP_MAX ? USLEEP_MAX : usec;
        if (usleep(chunk) == -1) {
            return -1;
        }
        usec -= chunk;
 ...
 *  first freed.
 */
int updateStringValue(TCHAR **ptr, const TCHAR *value) {
    if (*ptr != NULL) {
        free(*ptr);
        *ptr = NULL;
    }
 ...
 */
void clearNonAlphanumeric(const TCHAR* bufferIn, TCHAR* bufferOut) {
    while (*bufferIn) {
 ...
 */
static int encodingIsCanonicalName(const TCHAR* encoding) {
    TCHAR c;
    int i;
 ...
 */
int compareEncodings(const TCHAR* encoding1, const TCHAR* encoding2, int ignoreCase, int ignorePunctuation) {
 ...
 * @return TRUE if the encodings are identical, FALSE otherwise.
 */
int compareEncodingsSysMode(const TCHAR* encoding1, const TCHAR* encoding2) {
    ...
    *outputBufferMB = NULL;
    ...
    if (req <= 0) {
        ...
        *outputBufferMB = malloc(errorTemplateLen);
        if (*outputBufferMB) {
            _snprintf(*outputBufferMB, errorTemplateLen, errorTemplate, GetLastError());
        } else {
            /* Out of memory. *outputBufferW already NULL. */
        }
        return -1;
...
int converterWideToMB(const TCHAR *wideChars, char **outputBufferMB, const char *outputEncoding) {
    ...
    *outputBufferMB = NULL;
    ...
    if (len == (size_t)-1) {
        ...
        *outputBufferMB = malloc(errorTemplateLen);
        if (*outputBufferMB) {
            strncpy(*outputBufferMB, errorTemplate, errorTemplateLen);
        } else {
            /* Out of memory. *outputBufferW already NULL. */
        }
        return -1;
...
/**
 * Gets the error code for the last socket operation that failed.
 */
int wrapperGetSocketLastError() {
#ifdef WIN32
