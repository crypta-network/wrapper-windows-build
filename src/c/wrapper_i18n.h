...
#else
extern int _tcsnicmp(const wchar_t *string1, const wchar_t *string2, size_t count);
#endif
...
extern int multiByteToWideChar(const char *multiByteChars, int encoding, TCHAR **outputBufferW, int localizeErrorMessage);
 #define converterMBToWide(m, e, o, l)  multiByteToWideChar(m, e, o, l)
#endif

#define COUNT_DIGITS(n)     ((int)(n == 0 ? 1 : (n < 0 ? 1 : 0) + floor(log10(abs(n))) + 1))
...
#define STATUS_UNKNOWN    3
 ...
 #ifdef HPUX
  #define MB_UTF8   "utf8"
  #define W_UTF8    TEXT("utf8")
 #else
  #define MB_UTF8   "UTF-8"
  #define W_UTF8    TEXT("UTF-8")
 #endif
...
extern int updateStringValue(TCHAR **ptr, const TCHAR *value);

extern void clearNonAlphanumeric(const TCHAR* bufferIn, TCHAR* bufferOut);

extern int compareEncodings(const TCHAR* encoding1, const TCHAR* encoding2, int ignoreCase, int ignorePunctuation);

extern int compareEncodingsSysMode(const TCHAR* encoding1, const TCHAR* encoding2);
 ...
 */
extern int wrapperGetSocketLastError();
